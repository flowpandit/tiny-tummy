...
IOS_TARGETED_DEVICE_FAMILY="1,2"
...
if grep -q "TARGETED_DEVICE_FAMILY" "$PBXPROJ_PATH"; then
  perl -0pi -e "s/TARGETED_DEVICE_FAMILY = [^;]+;/TARGETED_DEVICE_FAMILY = \"$IOS_TARGETED_DEVICE_FAMILY\";/g" "$PBXPROJ_PATH"
else
  perl -0pi -e "s/(PRODUCT_BUNDLE_IDENTIFIER = $IOS_BUNDLE_IDENTIFIER;\n)/\${1}\t\t\t\tTARGETED_DEVICE_FAMILY = \"$IOS_TARGETED_DEVICE_FAMILY\";\n/g" "$PBXPROJ_PATH"
fi
...
if [ -f "$PROJECT_YML_PATH" ]; then
  ...
  if grep -q "TARGETED_DEVICE_FAMILY:" "$PROJECT_YML_PATH"; then
    perl -0pi -e "s/TARGETED_DEVICE_FAMILY: .+/TARGETED_DEVICE_FAMILY: \"$IOS_TARGETED_DEVICE_FAMILY\"/g" "$PROJECT_YML_PATH"
  else
    perl -0pi -e "s/(PRODUCT_BUNDLE_IDENTIFIER: $IOS_BUNDLE_IDENTIFIER\n)/\${1}      TARGETED_DEVICE_FAMILY: \"$IOS_TARGETED_DEVICE_FAMILY\"\n/g" "$PROJECT_YML_PATH"
  fi
  perl -0pi -e "s/(\n\s+iOS: )([0-9.]+)/\${1}$IOS_DEPLOYMENT_TARGET/g" "$PROJECT_YML_PATH"
fi
